...
class VoteViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if participant == 0 or evolution_criteria == 0:
            return Response({'detail': 'Ma\'lumotlar yetarli emas!'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if unical_code:
            try:
                ...
                if check_vote:
                    return Response({status:True, 'detail': 'Siz avval ovoz bergansiz'}, status=status.HTTP_404_NOT_FOUND)
                Vote.objects.create(
                ...
                return Response({'detail': 'Ovoz berildi!'}, status=status.HTTP_200_OK)
            except ObjectDoesNotExist:
                return Response({'detail': 'Kod aniqlanmadi!'}, status=status.HTTP_400_BAD_REQUEST)
        else:
            return Response({'detail': 'Kod kiritilmadi!'}, status=status.HTTP_400_BAD_REQUEST)
...
class BotLinkViewSet(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        return Response({'link': f'https://t.me/{bot_username}?start={unical_code}', 'code': unical_code}, status=status.HTTP_200_OK)
        ...
        if unical_code:
            try:
                ...
                if not subscriber.exists():
                    return Response({'status':False,'detailA': "Kanalga a'zolik aniqlanmadi"}, status=status.HTTP_404_NOT_FOUND)

                return Response({'status':True,'detail': 'Success'}, status=status.HTTP_200_OK)
            except UnicalCode.DoesNotExist:
                return Response({'status': False,'detail':"Xatolik"}, status=status.HTTP_404_NOT_FOUND)
